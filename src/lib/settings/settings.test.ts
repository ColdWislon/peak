...
describe('parseSettings', () => {
  it('boucle avec serializeSettings', () => {
    const settings = { quality: 'eco', units: 'imperial' } as const;
  ...
  it('retombe sur les défauts pour null, JSON cassé ou valeurs inconnues', () => {
    ...
    expect(parseSettings('{"quality":"ultra","units":"coudées"}')).toEqual(DEFAULT_SETTINGS);
  });

  it('complète les champs manquants sans toucher aux valides', () => {
    expect(parseSettings('{"units":"imperial"}')).toEqual({ quality: 'auto', units: 'imperial' });
  });
